...
type Idempotency interface {
	...
	GetDaemonSet(namespace, name string) (*apps.DaemonSet, error)
	CreateOrUpdateDaemonSet(ds *apps.DaemonSet) error
	GetStatefulSet(namespace, name string) (*apps.StatefulSet, error)
	CreateOrUpdateStatefulSet(sts *apps.StatefulSet) error
...
func NewKubeIdempotency(kubeConfig string) Idempotency {
	cfg, err := getKubeConfig(kubeConfig)
	if err != nil {
		logger.Error("unable to create a kubeconfig: %v", err)
		return nil
	}
	client, err := kubernetes.NewForConfig(cfg)
	if err != nil {
		logger.Error("unable to create a Kubernetes client: %v", err)
		return nil
	}
...
func getKubeConfig(kubeconfig string) (*rest.Config, error) {
	...
	if err != nil {
		// 使用 InClusterConfig，如果在集群内部运行，这将自动检测
...
// GetDeployment returns a Deployment object for the given namespace and name
func (ki *kubeIdempotency) GetDeployment(namespace, name string) (*apps.Deployment, error) {
	return ki.client.AppsV1().Deployments(namespace).Get(context.TODO(), name, metav1.GetOptions{})
}

func (ki *kubeIdempotency) GetDaemonSet(namespace, name string) (*apps.DaemonSet, error) {
	return ki.client.AppsV1().DaemonSets(namespace).Get(context.TODO(), name, metav1.GetOptions{})
}

func (ki *kubeIdempotency) GetStatefulSet(namespace, name string) (*apps.StatefulSet, error) {
	return ki.client.AppsV1().StatefulSets(namespace).Get(context.TODO(), name, metav1.GetOptions{})
}
...
func (ki *kubeIdempotency) CreateOrUpdateStatefulSet(sts *apps.StatefulSet) error {
	if _, err := ki.client.AppsV1().StatefulSets(sts.ObjectMeta.Namespace).Create(context.TODO(), sts, metav1.CreateOptions{}); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return fmt.Errorf("unable to create daemonset: %w", err)
		}

		if _, err := ki.client.AppsV1().StatefulSets(sts.ObjectMeta.Namespace).Update(context.TODO(), sts, metav1.UpdateOptions{}); err != nil {
			return fmt.Errorf("unable to update sts: %w", err)
		}
	}
	return nil
}
